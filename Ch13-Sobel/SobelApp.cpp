...
struct RenderItem
{
	...
	// Thus, when we modify obect data we should set `NumFramesDirty=gNumFrameResources`
...
void SobelApp::BuildLandGeometry()
{
	...
	// In addition, color the vertices based on their height
	// so we have sandy looking beaches, grassy low hills, and snow mountain peaks.
...
void SobelApp::BuildFrameResources()
{
	for (int i = 0; i < gNumFrameResources; ++i)
	{
		mFrameResources.emplace_back(std::make_unique<FrameResource>(
			md3dDevice.Get(), 1, (UINT)mAllRitems.size(), (UINT)mMaterials.size(), InitializeType::material
		));
	}
}
...
void SobelApp::BuildRenderItems()
{
	...
	mRitemLayer[(int)RenderLayer::GpuWaves].emplace_back(wavesRitem.get());
