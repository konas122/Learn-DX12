...
void CubeMapApp::BuildRenderItems()
{
	...
	mRitemLayer[(int)RenderLayer::OpaqueDynamicReflectors].emplace_back(globeRitem.get());
	mAllRitems.emplace_back(std::move(globeRitem));
...
void CubeMapApp::BuildCubeFaceCamera(float x, float y, float z)
{
	...
	// so we need a different "up" vector.
	XMFLOAT3 ups[6] =
	{
